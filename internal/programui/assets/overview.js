...
function programCard(program) {
  ...
  link.setAttribute("aria-label", `Open ${program.display_title}`);
  ...
  const queue = make(
    "p",
    "program-meta",
    `${program.ready} ready · ${program.progress.completed} of ${program.progress.total} complete`,
  );
  link.append(top, title, summary, stats, progress, next, queue);
  return link;
}
...
function workCard(item) {
  ...
  link.setAttribute(
    "aria-label",
    `Open ${item.id}, ${item.title}, in ${item.program_title}`,
  );
  ...
  link.append(
    meta,
    make("h4", "", item.title),
    make("p", "work-card__program", item.program_title),
  );
