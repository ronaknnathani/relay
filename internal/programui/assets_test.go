...
func TestEmbeddedAssetsStayLocalAndSemantic(t *testing.T) {
	...
	if strings.Count(index, "<script") != 3 ||
		...
		t.Error("index.html must embed the initial roadmap before the external application bundle")
	}
	if strings.Count(index, "<link ") != 1 ||
		!strings.Contains(index, `<link rel="stylesheet" href="/app.css">`) {
		t.Error("index.html must load the cacheable core stylesheet")
	}
}
...
func TestIndexBootstrapsTheLightThemeBeforePaint(t *testing.T) {
	...
	if !strings.Contains(script, "function start()") ||
		...
		t.Error("app.js must start immediately because the external bundle follows the complete document body")
	}
	requireContains(t, "app.js", script, []string{
	...
	if !strings.Contains(index, `<link rel="stylesheet" href="/app.css">`) ||
		!strings.Contains(index, `<script src="/app.js"></script>`) {
		t.Error("the stable core assets must be cacheable same-origin resources")
	}
