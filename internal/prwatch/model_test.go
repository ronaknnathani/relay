...
func TestParseOutcome(t *testing.T) {
	for _, value := range []string{"handled", "escalated", "obsolete"} {
		outcome, err := ParseOutcome(value)
		if err != nil {
			t.Fatalf("ParseOutcome(%q) = %v", value, err)
		}
		if string(outcome) != value {
			t.Errorf("ParseOutcome(%q) = %q", value, outcome)
		}
	}
	if _, err := ParseOutcome("done"); err == nil {
		t.Error("ParseOutcome(\"done\") = nil error, want rejection")
	}
}

func TestWithAcknowledgedKeysDedupesAndBounds(t *testing.T) {
	state := State{}.WithAcknowledgedKeys([]string{"a", "b", "a", ""})
	want := []string{"a", "b"}
	if len(state.AcknowledgedKeys) != len(want) {
		t.Fatalf("AcknowledgedKeys = %v, want %v", state.AcknowledgedKeys, want)
	}
	for i, key := range want {
		if state.AcknowledgedKeys[i] != key {
			t.Fatalf("AcknowledgedKeys = %v, want %v", state.AcknowledgedKeys, want)
		}
	}
	if !state.Acknowledged("b") {
		t.Error("Acknowledged(\"b\") = false, want true")
	}
	if state.Acknowledged("c") {
		t.Error("Acknowledged(\"c\") = true, want false")
	}

	overflow := make([]string, MaxAcknowledgedKeys+10)
	for i := range overflow {
		overflow[i] = "key-" + string(rune('a'+i%26)) + "-" + time.Duration(i).String()
	}
	bounded := State{}.WithAcknowledgedKeys(overflow)
	if len(bounded.AcknowledgedKeys) != MaxAcknowledgedKeys {
		t.Errorf("bounded keys = %d, want %d", len(bounded.AcknowledgedKeys), MaxAcknowledgedKeys)
	}
	if bounded.AcknowledgedKeys[MaxAcknowledgedKeys-1] != overflow[len(overflow)-1] {
		t.Error("bounded keys dropped the newest key instead of the oldest")
	}
}
